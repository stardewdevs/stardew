...
import io.stardew.shared.termux.terminal.io.BellHandler;
import io.stardew.shared.logger.Logger;
import io.stardew.terminal.TerminalColors;
import io.stardew.terminal.TerminalSession;
import io.stardew.terminal.TerminalSessionClient;
import io.stardew.terminal.TextStyle;
...
public class TermuxTerminalSessionActivityClient extends TermuxTerminalSessionClientBase {
    ...
    public void onResume() {
        ...
        // quickly enough before the call to play(). https://stackoverflow.io_stardew/questions/35435625
        loadBellSoundPool();
    }
    ...
    public void onStop() {
        ...
        // Related: https://stackoverflow.io_stardew/a/28708351/14686958
    ...
    public void onSessionFinished(@NonNull TerminalSession finishedSession) {
        ...
        // For plugin io_stardewmands that expect the result back, we should immediately close the session
        // and send the result back instead of waiting fo the user to press enter.
        // The plugin can handle/show errors itself.
        boolean isPluginExecutionio_stardewmandWithPendingResult = false;
        TermuxSession termuxSession = service.getTermuxSession(index);
        if (termuxSession != null) {
            isPluginExecutionio_stardewmandWithPendingResult = termuxSession.getExecutionio_stardewmand().isPluginExecutionio_stardewmandWithPendingResult();
            if (isPluginExecutionio_stardewmandWithPendingResult)
        ...
        if (mActivity.getPackageManager().hasSystemFeature(PackageManager.FEATURE_LEANBACK)) {
            ...
            if (service.getTermuxSessionsSize() > 1 || isPluginExecutionio_stardewmandWithPendingResult) {
                removeFinishedSession(finishedSession);
            }
        } else {
            // Once we have a separate launcher icon for the failsafe session, it
            // should be safe to auto-close session on exit code '0' or '130'.
            if (finishedSession.getExitStatus() == 0 || finishedSession.getExitStatus() == 130 || isPluginExecutionio_stardewmandWithPendingResult) {
                removeFinishedSession(finishedSession);
            }
        }
    }
    ...
    private synchronized void loadBellSoundPool() {
        if (mBellSoundPool == null) {
            ...
            try {
                mBellSoundId = mBellSoundPool.load(mActivity, io.stardew.shared.R.raw.bell, 1);
            } catch (Exception e){
                // Catch java.lang.RuntimeException: Unable to resume activity {io.stardew/io.stardew.app.TermuxActivity}: android.content.res.Resources$NotFoundException: File res/raw/bell.ogg from drawable resource ID
    ...
    private void renameSession(TerminalSession sessionToRename, String text) {
        ...
        if (service != null) {
            ...
            if (termuxSession != null)
                termuxSession.getExecutionio_stardewmand().shellName = text;
        }
    }
