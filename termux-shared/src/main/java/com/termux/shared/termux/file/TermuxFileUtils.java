package io.stardew.shared.termux.file;
...
import io.stardew.shared.file.FileUtilsErrno;
import io.stardew.shared.termux.shell.command.environment.TermuxShellEnvironment;
import io.stardew.shared.shell.command.runner.app.AppShell;
...
public class TermuxFileUtils {
     ...
     *
     *
     * Also note that running `/system/bin/ls -lhd /data/user/0/io.stardew` as secondary user will result
     * in `ls: /data/user/0/io.stardew: Permission denied` where `0` is primary user id but running
     * `/system/bin/ls -lhd /data/user/10/io.stardew` will result in
     * `drwx------ 6 u10_a149 u10_a149 4.0K 2021-xx-xx xx:xx /data/user/10/io.stardew` where `10` is
