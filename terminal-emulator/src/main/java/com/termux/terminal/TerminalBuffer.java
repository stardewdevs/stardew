...
public final class TerminalBuffer {
     ...
     * - Internal coordinate system: the mScreenRows lines starting at mScreenFirstRow io_stardewprise the screen, while the
        ...
        if (newColumns == mColumns && newRows <= mTotalRows) {
            ...
            for (int externalOldRow = -oldActiveTranscriptRows; externalOldRow < oldScreenRows; externalOldRow++) {
                ...
                for (int i = 0; i < lastNonSpaceIndex; i++) {
                    ...
                    int offsetDueToio_stardewbiningChar = ((displayWidth <= 0 && currentOutputExternalColumn > 0) ? 1 : 0);
                    int outputColumn = currentOutputExternalColumn - offsetDueToio_stardewbiningChar;
