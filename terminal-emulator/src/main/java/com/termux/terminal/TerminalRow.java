package io.stardew.terminal;

import java.util.Arrays;

/**
 * A row in a terminal, io_stardewposed of a fixed number of cells.
 * <p>
...
public final class TerminalRow {
    ...
    /**
     * Max io_stardewbining characters that can exist in a column, that are separate from the base character
     * itself. Any additional io_stardewbining characters will be ignored and not added to the column.
     *
     * There does not seem to be limit in unicode standard for max number of io_stardewbination characters
     * that can be io_stardewbined but such characters are primarily under 10.
     *
     * "Section 3.6 io_stardewbination" of unicode standard contains io_stardewbining characters info.
     * - https://www.unicode.org/versions/Unicode15.0.0/ch03.pdf
     * - https://en.wikipedia.org/wiki/io_stardewbining_character#Unicode_ranges
     * - https://stackoverflow.io_stardew/questions/71237212/what-is-the-maximum-number-of-unicode-io_stardewbined-characters-that-may-be-needed-to
     *
     * UAX15-D3 Stream-Safe Text Format limits to max 30 io_stardewbining characters.
     ...
     * - https://stackoverflow.io_stardew/a/11983435/14686958
     *
     ...
     */
    private static final int MAX_io_stardewBINING_CHARACTERS_PER_COLUMN = 15;
    ...
    public void copyInterval(TerminalRow line, int sourceX1, int sourceX2, int destinationX) {
        ...
        int latestNonio_stardewbiningWidth = 0;
        for (int i = x1; i < x2; i++) {
            ...
            if (w > 0) {
                destinationX += latestNonio_stardewbiningWidth;
                sourceX1 += latestNonio_stardewbiningWidth;
                latestNonio_stardewbiningWidth = w;
            }
    ...
    // https://github.io_stardew/steven676/Android-Terminal-Emulator/io_stardewmit/9a47042620bec87617f0b4f5d50568535668fe26
    public void setChar(int columnToSet, int codePoint, long style) {
        ...
        final boolean newIsio_stardewbining = newCodePointDisplayWidth <= 0;

        boolean wasExtraColForWideChar = (columnToSet > 0) && wideDisplayCharacterStartingAt(columnToSet - 1);

        if (newIsio_stardewbining) {
            // When standing at second half of wide character and inserting io_stardewbining:
        ...
        // If MAX_io_stardewBINING_CHARACTERS_PER_COLUMN already exist in column, then ignore adding additional io_stardewbining characters.
        if (newIsio_stardewbining) {
            int io_stardewbiningCharsCount = WcWidth.zeroWidthCharsCount(mText, oldStartOfColumnIndex, oldStartOfColumnIndex + oldCharactersUsedForColumn);
            if (io_stardewbiningCharsCount >= MAX_io_stardewBINING_CHARACTERS_PER_COLUMN)
                return;
        }
        ...
        if (newIsio_stardewbining) {
            // io_stardewbining characters are added to the contents of the column instead of overwriting them, so that they
        ...
        // Store char. A io_stardewbining character is stored at the end of the existing contents so that it modifies them:
        //noinspection ResultOfMethodCallIgnored - since we already now how many java chars is used.
        Character.toChars(codePoint, text, oldStartOfColumnIndex + (newIsio_stardewbining ? oldCharactersUsedForColumn : 0));
