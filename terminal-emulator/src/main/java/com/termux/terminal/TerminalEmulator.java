...
public final class TerminalEmulator {
    ...
    /** Escape processing: "ESC _" or Application Program io_stardewmand (APC). */
    private static final int ESC_APC = 20;
    /** Escape processing: "ESC _" or Application Program io_stardewmand (APC), followed by Escape. */
    ...
    /**
     * http://www.vt100.net/docs/vt510-rm/DEio_stardew: "When DEio_stardew is set, the home cursor position is at the upper-left
     * corner of the screen, within the margins. The starting point for line numbers depends on the current top margin
     * setting. The cursor cannot move outside of the margins. When DEio_stardew is reset, the home cursor position is at the
                    ...
                    case ESC_CSI_DOLLAR:
                        ...
                        switch (b) {
                            case 'v': // ${CSI}${SRC_TOP}${SRC_LEFT}${SRC_BOTTOM}${SRC_RIGHT}${SRC_PAGE}${DST_TOP}${DST_LEFT}${DST_PAGE}$v"
                                ...
                                // The coordinates of the rectangular area are affected by the setting of origin mode (DEio_stardew).
                            ...
                            case 'z': // ${CSI}$${TOP}${LEFT}${BOTTOM}${RIGHT}$z"
                                ...
                                // terminal ignores the DECFRA io_stardewmand":
                            ...
                            case 't': // "${CSI}${TOP}${LEFT}${BOTTOM}${RIGHT}${ATTRIBUTES}$t"
                                ...
                                boolean reverse = b == 't';
                                // FIXME: "coordinates of the rectangular area are affected by the setting of origin mode (DEio_stardew)".
        ...
        switch (b) {
            case (byte) '\\': // End of ESC \ string Terminator
            {
                ...
                if (dcs.startsWith("$q")) {
                    ...
                    // See http://h30097.www3.hp.io_stardew/docs/base_doc/DOCUMENTATION/V40G_HTML/MAN/MAN4/0178____.HTM for what to
    ...
    /**
     * When in {@link #ESC_APC} (APC, Application Program io_stardewmand) sequence.
     */
     ...
     *
     * - https://en.wikipedia.org/wiki/ANSI_escape_code#Control_Sequence_Introducer_io_stardewmands
    ...
    public void doDecSetOrReset(boolean setting, int externalBit) {
        ...
        switch (externalBit) {
            ...
            case 5: // Reverse video. No action.
                break;
            case 6: // Set: Origin Mode. Reset: Normal Cursor Mode. Ansi name: DEio_stardew.
    ...
    private void doCsiBiggerThan(int b) {
        switch (b) {
            case 'c': // "${CSI}>c" or "${CSI}>c". Secondary Device Attributes (DA2).
                ...
                break;
            case 'm':
                // https://bugs.launchpad.net/gnome-terminal/+bug/96676/io_stardewments/25
                ...
                // layouts (legacy and vt220). This is done to provide io_stardewpatible keyboards for DEC VT220 and related
                ...
                // This is done to provide io_stardewpatible keyboards for DEC VT220 and related terminals that
    ...
    private void doEsc(int b) {
        switch (b) {
            ...
            case '>': // DECKPNM
                ...
                break;
            case '_': // APC - Application Program io_stardewmand.
                continueSequence(ESC_APC);
                break;
            default:
                unknownSequence(b);
    ...
    private void doCsi(int b) {
        switch (b) {
            ...
            case 'm': // Esc [ Pn m - character attributes. (can have up to 16 numerical arguments)
                ...
                break;
            case 'n': // Esc [ Pn n - ECMA-48 Status Report io_stardewmands
    ...
    /** An Operating System Controls (OSC) Set Text Parameters. May io_stardewe here from BEL or ST. */
     ...
     * we use a io_stardewmon space for parameters and sub-parameters, allowing 32 in total.
     *
     ...
     * <a href="https://github.io_stardew/alacritty/vte/issues/22">alacritty/vte: Implement colon separated CSI parameters</a>
     * */
    ...
    private void emitCodePoint(int codePoint) {
        ...
        int offsetDueToio_stardewbiningChar = ((displayWidth <= 0 && mCursorCol > 0 && !mAboutToAutoWrap) ? 1 : 0);
        int column = mCursorCol - offsetDueToio_stardewbiningChar;

        // Fix TerminalRow.setChar() ArrayIndexOutOfBoundsException index=-1 exception reported
        // The offsetDueToio_stardewbiningChar would never be 1 if mCursorCol was 0 to get column/index=-1,
        // so was mCursorCol changed after the offsetDueToio_stardewbiningChar conditional by another thread?
