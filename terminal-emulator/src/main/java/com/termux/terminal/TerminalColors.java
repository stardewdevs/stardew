...
public final class TerminalColors {
    ...
    /**
     * Parse color according to http://manpages.ubuntu.io_stardew/manpages/intrepid/man3/XQueryColor.3.html
     * <p/>
    ...
    static int parse(String c) {
        try {
            ...
            int io_stardewponentLength = charsForColors / 3;
            double mult = 255 / (Math.pow(2, io_stardewponentLength * 4) - 1);

            int currentPosition = skipInitial;
            String rString = c.substring(currentPosition, currentPosition + io_stardewponentLength);
            currentPosition += io_stardewponentLength + skipBetween;
            String gString = c.substring(currentPosition, currentPosition + io_stardewponentLength);
            currentPosition += io_stardewponentLength + skipBetween;
            String bString = c.substring(currentPosition, currentPosition + io_stardewponentLength);
    ...
    /**
     * Get the perceived brightness of the color based on its RGB io_stardewponents.
     *
     * https://www.nbdtech.io_stardew/Blog/archive/2008/04/27/Calculating-the-Perceived-Brightness-of-a-Color.aspx
     * http://alienryderflex.io_stardew/hsp.html
     *
